...
const SectionFounder = () => {
  return (
    <div className=" hebrew nc-SectionFounder relative">
      <Heading desc="אנחנו שלטי דרום, כאן, זמינים לתת לכם את השירות הכי טוב שאפשר">
        ⛱ הצוות שלנו
      </Heading>
        ...
        {FOUNDER_DEMO.map((item) => (
          <div key={item.id} className="max-w-sm">
            <NcImage
              ...
              src={item.avatar}
            />
            <h3 className=" hebrew text-lg font-semibold text-neutral-900 mt-4 md:text-xl dark:text-neutral-200">
              {item.name}
            </h3>
            <span className=" hebrew block text-sm text-neutral-500 sm:text-base dark:text-neutral-400">
              {item.job}
            </span>
          </div>
        ))}
